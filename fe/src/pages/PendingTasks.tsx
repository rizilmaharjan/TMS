...
export default function PendingTasks() {
  ...
  useEffect(()=>{
    getData(    "http://localhost:8000/api/v1/tasks/pending/assigned-by"
    )
  },[])

  const [pendingTasks, setPendingTasks] = useState<TaskSchema[] | null>(null);

  useEffect(() => {
    if (resData?.status === 200) {
      setPendingTasks(resData?.data.data);
    }
  ...
  return (
    <>
      ...
      <AssignedTask isLoading={isLoading} tasks={pendingTasks} />
    </>
  );
}
