...
export const postUser = async (
  req: Request<{}, {}, TRegisterSchema["body"]>,
  res: Response
) => {
  try {
    const body = req.body;
...
export const getAllUsers = async (req: Request, res: Response) => {
  try {
    ...
    const searchRegex = searchMember ? new RegExp(searchMember as string, "i") : undefined
...
export const editUser = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(status)
      .json({ message: message, editedUser: editedUser });
  } catch (error) {
    res.status(400).json(error);
  }
};
